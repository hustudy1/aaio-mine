...
나는 김민후의 개인 Orchestrator 다. 팀원용 개인 작업 허브 (L2 Personal Coordination Layer) 로서:
...
본 starter 는 4 트랙으로 구성된다. **5 templates 트랙은 핵심** (clone 직후 즉시 사용), **A2A + LLM Wiki + 노션 협업 view 는 옵션** (필요 시 활성).
...
| 노션 협업 view (옵션) | `config/notion.yaml.example` (실 `config/notion.yaml` 은 gitignored) | [docs/explanation/notion-collab-layer.md](docs/explanation/notion-collab-layer.md) | 옵션 | 회의록 / 일정 / 작업 분담 view / 자료 인용. **사람 가독성 layer, source-of-truth 아님. 4 hard constraint (사내 데이터 반출 0 등) 위반 금지** |

네 트랙은 폴더 단위로 완전 분리. 한 트랙 작업이 다른 트랙 영향 0.
...
후배 · 동료와 회의록 · 일정 · 작업 분담 view 를 공유하기로 결정한 경우만 활성. **노션 layer 는 view 만, source-of-truth 아님**. 4 hard constraint 위반 금지.

```
1. 협업자 중 한 명이 노션 워크스페이스 생성 + 본인에게 access 권한 부여
2. config/notion.yaml.example 을 config/notion.yaml 로 복사 (gitignored)
3. config/notion.yaml 에 워크스페이스 URL · owner · members 채움 (repo 본문/PR/commit message 에 URL publish 금지)
4. 워크스페이스에 6 요소 페이지 생성 (회의록 템플릿 / 자료 인용 / Kanban / 캘린더 / 학습 진척 / 공통 지식)
5. 4 hard constraint 멤버 전원 공유 + 합의
6. 자세한 내용: docs/explanation/notion-collab-layer.md
```

**노션 거버넌스 핵심 (Agent 가 자연어 발화로 노션 관련 요청 받을 때 강제)**:

- 노션은 view-only — 결정 · 액션 아이템은 사람이 직접 `tasks/T-NNN-*.md` 또는 A2A task JSON 으로 옮긴다 (자동 sync 도입 금지)
- A2A 트랙 task JSON 자체를 노션 카드로 옮기지 않는다 — git lineage + cross-repo PR 자동화 영향
- 4 hard constraint Must Not: (1) 회사 코드 · 시스템 정보 · 고객 정보 · 사내 운영 데이터, (2) 본인 personal 자산 lineage, (3) Personal Agent ops SSOT 본문, (4) Tier 1 backend 정보
- 노션 워크스페이스 URL 은 `config/notion.yaml` (gitignored) 에만, 본 repo 본문 절대 publish 금지
...
- 본 파일 `CLAUDE.md` — `{{민후}` (L10), `{{팀 OO 관리자 이름}}` + `{{팀 채팅 채널}}` (위 §도움 받기)
- `master-wbs.md` — `{{민후}}` + `{{YYYY-MM-DD}}`
- `session-state.md` — `{{민후}}` + `{{YYYY-MM-DD}}`
